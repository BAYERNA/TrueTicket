        ...
        <>
          ...
          {!payment && (
            <button
              ...
              className="rounded-full bg-blue-700 px-5 py-2.5 text-sm font-medium text-white disabled:opacity-50"
            >
          ...
          {payment?.status === "PENDING" && (
            <div className="flex flex-col gap-3 rounded-lg border border-zinc-200 p-4 dark:border-zinc-800">
              ...
              <p className="text-xs text-zinc-500">
                개발 환경의 모의 결제입니다. 운영에서는 PG 결제창과 서명 검증 웹훅으로 대체됩니다.
              </p>
              <button
                type="button"
          ...
          {payment?.status === "PAID" && (
            <Link
              href="/my-tickets"
              className="rounded-full bg-green-700 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-green-800"
            >
              결제 완료 — 마이 티켓 보기
            </Link>
          )}
          {(prepare.isError || complete.isError) && (
            <p className="text-sm text-red-600">
              결제를 처리할 수 없습니다. 선점 시간이 만료되었는지 확인해주세요.
            </p>
          )}
        </>
      ) : (
